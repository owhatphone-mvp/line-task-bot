...
=== รูปแบบการตอบ ===
- พูดเป็นกันเอง สุภาพ กระชับ ไม่ยาวเกินไป
- ตอบเป็นภาษาไทยเสมอ ยกเว้นถูกถามเป็นภาษาอื่น
- คำตอบทั่วไป: ห้ามยาวเกิน 800 ตัวอักษร ถ้าเรื่องยาวให้สรุปเป็นหัวข้อ
- คำตอบเชิงที่ปรึกษา (วางแผน/บริหาร/จัดงาน): ตอบได้ถึง 1500 ตัวอักษร เพื่อให้ครบถ้วน
- ถ้าเป็นแผนงานยาว ให้แบ่งเป็น Phase หรือ Step ชัดเจน
- ตอบทุกเรื่องที่ถามมาได้ ไม่ต้องปฏิเสธว่า "ผมเป็นแค่บอทจัดการงาน" คุณคือ AI เต็มรูปแบบ
- ถ้าคำถามเกี่ยวข้องกับงานในระบบ ให้อ้างอิงข้อมูลงานที่มีอยู่ด้วย
- ถ้าไม่แน่ใจคำตอบ ให้บอกตรงๆ ว่าไม่แน่ใจ อย่าแต่งคำตอบขึ้นมา`;
...
async function handleJoinEvent(event) {
  ...
  await replyMessage(event.replyToken, 
    `👋 สวัสดีครับทุกคน! ผม "อู่อันโหว" ผู้ช่วยจัดการงานของกลุ่มนี้ครับ\n\n` +
    `🔥 ก่อนเริ่ม! เพิ่มผมเป็นเพื่อนก่อนนะครับ\n` +
    `👉 https://line.me/R/ti/p/@909kiqvu\n\n` +
    `เพราะผมจะส่งแจ้งเตือนงาน รับงาน ส่งงาน ผ่าน DM ส่วนตัว\n` +
    `จะได้ไม่รกกลุ่มครับ! 😄\n\n` +
    `💡 คำสั่งในกลุ่ม:\n` +
    `📌 #สั่งงาน @ชื่อคน รายละเอียดงาน\n` +
    `📋 #สรุปงาน | #สรุปแชท\n` +
  ...
  for (const member of joinedMembers) {
    if (member.type === 'user') {
      ...
      await sendDM(member.userId, 
        `👋 ยินดีต้อนรับเข้ากลุ่มครับ!\n\n` +
        `ผม "อู่อันโหว" ผู้ช่วยจัดการงานครับ\n` +
        `ระบบจะส่งแจ้งเตือนงานมาทาง DM นี้เลย\n\n` +
        `📌 สั่งงาน → ใช้ในกลุ่ม\n` +
        `💪 รับงาน/ส่งงาน → ใช้ใน DM นี้\n` +
        `❓ #help - ดูคำสั่งทั้งหมด`
      );
    }
  }
